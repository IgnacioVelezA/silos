...
def curve_analizer(curve):
    ...
    distance = distanceFinder.distanceFinder(curve, 15, o, 30)
...
def animate(i, xs, ys):
    ...
    ys = np.array(curve)
    max_power = max(ys)
    distance, mean, std = curve_analizer(ys)
...
if __name__ == '__main__':
    ...
    ani = animation.FuncAnimation(fig, animate, fargs=(xs, ys), interval=(800))
    plt.show()
